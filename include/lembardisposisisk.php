...
if($_GET['action'] == "tampil"){
  ...
  $qlastidi=mysqli_query($mysqli,"SELECT id from disposisisk ORDER BY id DESC LIMIT 1") or die ("Error disini:".mysqli_error($mysqli));
    while($qr=mysqli_fetch_array($qlastidi)){
       $iddisposisi=$qr['id'];
    }
}
    
   $query = mysqli_query($mysqli, "SELECT * FROM disposisisk WHERE id='$iddisposisi'");
   $data = array();
   $no = 1;
