...
WHERE k.id='$_GET[pejabat_ttd]'")or die ("Error :".mysqli_error($mysqli));
		...
		$tugas=$_GET['tugas'];
		$hari=hari_indonesia($_GET['hari']);
		$tanggal=tgl_indonesia($_GET['tanggal_pelaksanaan']);
		$lama=$_GET['lama_tugas'];
		$tempat=$_GET['tempat'];
		$pembebanan_biaya=$_POST['pembebanan_biaya'];
		$tanggal_ttd=tgl_indonesia($_GET['tanggal_ttd']);
...
WHERE sptd.id='$_GET[id]'") or die("Error :".mysqli_error($mysqli));
                        	$no=1;
