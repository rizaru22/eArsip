...
$(function(){
  table = $('.table').DataTable({
      "processing" : true,
      "ajax" : {
         "url" :  "ajax/dbdisposisi.php?action=table_data",    //"ajax/ajax_ujian.php?action=table_data",
         "type" : "POST"
      }
   });
   $('#kirim').multiselect({
...
function form_datasurat(){
   ...
   $('#dataSuratMasuk form')[0].reset();
   $('.modal-title').text('Daftar Surat Masuk');
}
...
function daftar(){
		$('#isi').hide().load('view/daftarDisposisi.php').fadeIn('normal');
}

//Ketika tombol simpan diklik
function simpandata(){	
       url = "ajax/dbdisposisi.php?action=insert";
    $.ajax({
      url : url,
      ...
      success: function(data) {
         if(data=="ok"){
          $("#printabel").remove();
                ...
                .attr("src", "include/lembardisposisi.php?action=cetak") 
                .appendTo("body");  
          reset_form();
         }else{
            alert(data);
...
function pilih_data(id){
   ...
   $('#dataSuratMasuk form')[0].reset();
   $.ajax({
      url : "ajax/dbdisposisi.php?action=form_data&id="+id,
      type : "GET",
      dataType : "JSON",
      success : function(data){
         //$('#dataSuratMasuk').modal('show');
